...
def topk_ppr_matrix(adj_matrix, flag, alpha, eps, idx, topk, normalization='sym'):
    ...
    if flag:       
        row = adj_matrix.storage.row().cpu()
        col = adj_matrix.storage.col().cpu()
        data = adj_matrix.storage.value().cpu()
        sparse_sizes = (adj_matrix.size(0), adj_matrix.size(1))
        A = scipy.sparse.csr_matrix((data, (row, col)), shape=sparse_sizes)
    else:
