...
def train_on_UGC_models(data, num_classes, feature_size, hidden_units, learning_rate, decay, epochs, device, model_type="gcn"):
    if model_type == 'gin':
    ...
    if data.edge_attr == None:
        ...
        data.edge_attr = edge_weight.to(device=device)
...
def train_heterophilic_models(model, args, data, device):
        ...
        for epoch in range(args.epochs):
            ...
            if not args.sampling:
                ...
                if args.rocauc or args.dataset in ('yelp-chi', 'twitch-e', 'ogbn-proteins', 'genius'):
                    ...
                    loss = criterion(
                        out[train_idx], data.y.squeeze(1)[train_idx])
                loss.backward()
                optimizer.step()
            else:
                ...
                for batch_size, n_id, adjs in train_loader:
                    ...
                    optimizer.zero_grad()
                    out = model(data, adjs, data.x[n_id])
                    out = F.log_softmax(out, dim=1)
                    loss = criterion(out, data.y.squeeze(1)[n_id[:batch_size]])
                    loss.backward()
                    optimizer.step()
